...
def send_start_menu(bot, chat_id):
    ...
    keyboard.row(userbots_button, tg_bots_button)
...
def handle_buttons(message):
    ...
    if text == "Юзерботы":
        ...
        keyboard = telebot.types.ReplyKeyboardMarkup(resize_keyboard=True).row(hikka_telethon_button, rimtub_pyrogram_button)
        bot.send_message(chat_id, "<b>Выберите юзербота и библиотеку:</b>", reply_markup=keyboard)
    ...
    if text == "ТГ-боты":
        ...
        telethon_tgbots_button = telebot.types.KeyboardButtin(" Telethon")
        telebot_tgbots_button = telebot.types.KeyboardButton(" Telebot")
        keyboard = telebot.types.ReplyKeyboardMarkup(resize_keyboard=True).row(pyrogram_tgbots_button, telethob_tgbots_button, telebot_tgbots_button)
        bot.send_message(chat_id, "<b>Выберите библиотеку для написания бота:</b>", reply_markup=keyboard, parse_mode='HTML')
        elif text == "Pyrogram":
            article_text = "Статья: [Pyrogram-про то как написать простого телеграм бота](https://telegra.ph/Pyrogram-razrabotka-bota-10-30);"
            bot.send_message(chat_id, article_text, disable_web_page_preview=True, parse mode="Markdown")
        elif text == "Telethon":
