...
def send_welcome(client, chat_id, CHANNEL_USERNAME):
 try:
  chat_member = client.get_chat_member(CHANNEL_USERNAME, chat_id)
...
def send_start_menu(client, chat_id):
  keyboard = InlineKeyboardMarkup(
    [[InlineKeyboardButton(text="Юзерботы", callback_data="userbots"), InlineKeyboardButton(text="ТГ-боты", callback_data="tg_bots")]]
  )
  client.send_message(chat_id, "💫 Выберите направление:", reply_markup=keyboard, parse_mode='HTML')
